...
public class StructuredProgram {
	...
	static int[] kors = new int[3]; //전역변수로 만드는 방법인 static. 효율성은 나중에 따지자.
	//static Scanner scan = new Scanner(System.in);//이 방법은 좋은 방법이 아니다. scanner는 공유될 필요가 없기 때문. 굳이 공유할 필요가 없는 것은 각자 마련하게 하자.
//	static int total = 0; //이렇게쓰면 안된다. 전역변수로 쓰는 의미가 없기때문에. 
//	static float avg; //평균또한 마찬가지. 공통으로 이용되는 변수가 아니다.
